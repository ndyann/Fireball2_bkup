...
#define ACQ_PULSE		0x4		/**< deprecated, unsupported, do not use */
...
#define FG_PULSE_NEXT_BUFFER			0x230		/**< deprecated, unsupported, do not use */
 ...
 * placeholder for user defined event function argument.
 *
 * This struct is not defined anywhere within Silicon Software code, it is only passed
...
struct fg_event_info {
	...
	unsigned int notify[64];	///< bit 0 will be set to 1 if events were joined in mode FG_EVENT_BATCHED, bit 1 will be set if timestamp contains valid data
 ...
 *
 * \param events the mask of events that were received
 * \param data the context data passed to Fg_registerEventCallback()
...
enum FgEventControlFlags {
	...
	FG_EVENT_TIMESTAMP = 0x2	///< record the timestamp when the event was first seen (not implemented in version 5.1.2)
};
