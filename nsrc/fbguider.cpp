...
int main(){
  ...
  err=pthread_create(&message_thread, NULL, message_function, (void *) &message_data);
  ...
  while(1){
    ...
    DEBUGLINE
      err= pthread_create(&analysis_thread, NULL, image_function, (void *) &guide_data);
    pthread_detach(analysis_thread);
    printf("Image function address: 0x%x\n\t\tError: %d\n",&image_function, err);
